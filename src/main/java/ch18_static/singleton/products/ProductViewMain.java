...
//        ProductView productView2 = new ProductView();
        // private 선언 후에는 오류가 발생합니다.

        // 생성자를 private 처리를 했고, getInstance()메서드 내에서 instance 정적 변수가 null 이라면
        // instatnce = new ProductView();하기로 정의했었기 때문에 main 단계에서 저희는
        // ProductView.getInstnace();를 통해 객체를 생성합니다.
         ...
         /*
            이상의 코드에서 복수의 객체를 생성한 것 처럼 보이지만 실제로
            ProductView.getInstance(); 의 결과값은 동일한 instance입니다.(확인필요)
            그렇다면 실제 생성된 객체의 수는 '1개'입니다.
            그리고 그 한 개를 다른 객체 이름에 대입했을 뿐입니다.
