...
/*
    ...
    일반 for문과의 차이점 :
        ...
        일일이 element의 숫자를 세는것이 번거롭기때문에
        배열명.length와 같은 방식으로 int값을 추출해서 한계값에 대입하였다.

        그런데 잘 생각해보면 for(int i = 0; ...)으로 작성할 때
        int i 는 for문의 내부에서 선언됩니다. (일종의 지역변수라고 볼 수 있음)

        이를 응용하여 배열내의 element를 변수에 대입하는 지역 변수를 선언하는 방식으로
        index넘버의 사용 없이 바료 적용이 가능한 for 반복문이 있는데,
        이를 향상된 for문이라고 합니다.

        제약 :
            읽기만 되고, 쓰기가 안됩니다
            (예를 들어 1~100까지 추출하는건 되는데, 1~100까지 입력은 불가능할때가 있다)
...
//    }
    ...
    public void printElem(String[] names){
        ...
        for(String name : names){
            System.out.print(name + " / ");       // 출력문 쓰는 방법의 차이에 주목!!
        }
    }
