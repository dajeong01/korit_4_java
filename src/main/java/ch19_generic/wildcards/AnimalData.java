...
public class AnimalData<T>{
    ...
    /*
        ...
        그러면, Car는 들어가면 안될텐데, 현재의 상황으로는 Car가 대입되더라도 Java상에서 걸러낼 수 없습니다.

        특정 클래스(즉 Animal의 상속을 받은 서브 클래스에 해당된다면) 해당 클레스에 맞는 객체 정보를
        출력할 수 있도록 작성할 예정
     */
    public void printData(){
        ...
        if(animal.getClass() == Human.class){
            ...
            ((Tiger)animal).hunt();
        }   // 추후 다른 Animal의 서브 클래스를 만든다면 else if로 이어주면 됩니다.(했던 부분입니다)
    }
