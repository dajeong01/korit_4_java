...
public class StrMap {
    public static void main(String[] args) {
        ...
        // 특정 키의 값 수정 # 1 -> 아까본 것처럼 .put(키)를 통해 덮어쓰기
        ...
        System.out.println(entrySet2.size());   // 5가 출력된다는 점에서 'key값=value값'이 하나의 element로 집합
...
//        Set<String> valueSet = strMap1.values();  // 오류 발생 예시 -> 필요한 타입 Set / 제공된 타입 Collection
        // 이상에서 문제가 발생하는 이유는 key는 어짜피 중복을 허용하지않기때문에 Set으로 형변환이 가능한 반면
        // 복수의 key에서 동일한 value가 존재할 수 있기 때문에 Set으로 생성하는것이 불가능합니다.
        Collection<String> values = strMap1.values();
        System.out.println(values); // 그 와중에 .sort()할 떄처럼 collections가 아니라 collection
